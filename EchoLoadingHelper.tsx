...
const APP_VERSION = {
  tag: "v0.6",
  date: "2026-07-07",
  note: "IU chart: project before/after points to weight axis with Δweight bracket",
};
// changelog:
...
function IndexGraph({ plot, cg0, w0 }) {
  // x = Index Units, y = Weight (kg), weight increasing UP
  const Y0 = ECHO.MIN_W, Y1 = 2990;
  ...
  const W = 520, H = 380, padL = 58, padB = 40, padT = 16, padR = 14;
  const plotW = W - padL - padR, plotH = H - padT - padB;
  ...
  const envPath =
    `M ${fwdPts[0][0]} ${fwdPts[0][1]} ` +
    fwdPts.slice(1).map((p) => `L ${p[0]} ${p[1]}`).join(" ") +
    ` ` + aftPts.map((p) => `L ${p[0]} ${p[1]}`).join(" ") + " Z";

  const gridW = [2000, 2200, 2400, 2600, 2725, 2800, 2950];
  const iuFor = (w, cg) => (cg * w) / 10000;
  const before = plot?.before, after = plot?.after;
  const pB = before ? { x: sx(iuFor(before.w, before.cg)), y: sy(before.w), in: before.cg >= fwdLimit(before.w) && before.cg <= ECHO.AFT_LIMIT } : null;
  const pA = after ? { x: sx(iuFor(after.w, after.cg)), y: sy(after.w) } : null;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="eh-svg" role="img" aria-label="Echo index-vs-weight loading chart">
      <rect x={padL} y={padT} width={plotW} height={plotH} fill="rgba(255,255,255,0.02)" stroke="var(--line)" />
      {gridW.map((w) => (
        <g key={"w" + w}>
          <line x1={padL} y1={sy(w)} x2={W - padR} y2={sy(w)} stroke="var(--line)" strokeDasharray="2 4" opacity="0.5" />
      ...
      {xTicks.map((v) => (
        <g key={"x" + v}>
          <line x1={sx(v)} y1={padT} x2={sx(v)} y2={H - padB} stroke="var(--line)" strokeDasharray="2 4" opacity="0.35" />
          <text x={sx(v)} y={H - padB + 14} textAnchor="middle" className="eh-axis">{v}</text>
        </g>
      ))}
      <path d={envPath} fill="rgba(218,83,44,0.10)" stroke="var(--amber)" strokeWidth="2" />

      {/* boundary labels */}
      <text x={sx(iuFwd(2600)) - 4} y={sy(2600)} textAnchor="end" className="eh-axis" fill="var(--amber2)">fwd limit</text>
      <text x={sx(iuAft(2600)) + 4} y={sy(2600)} textAnchor="start" className="eh-axis" fill="var(--amber2)">aft limit</text>
      ...
      {/* Projections to the weight axis: read the weight difference = load to add/remove */}
      {pB && pA && (() => {
        ...
        return (
          <g>
            {/* horizontal dashed lines from each point across to the weight axis */}
            <line x1={padL} y1={pB.y} x2={pB.x} y2={pB.y} stroke="var(--dim)" strokeDasharray="3 3" strokeWidth="1" opacity="0.85" />
            <line x1={padL} y1={pA.y} x2={pA.x} y2={pA.y} stroke="var(--amber2)" strokeDasharray="3 3" strokeWidth="1" opacity="0.9" />
            {/* weight readings at the axis */}
            <text x={padL + 3} y={pB.y - 3} className="eh-pt" fill="var(--dim)">{fmt(before.w, 0)}</text>
            <text x={padL + 3} y={pA.y - 3} className="eh-pt" fill="var(--amber2)">{fmt(after.w, 0)}</text>
            {/* Δweight bracket just inside the axis */}
            <line x1={padL + 44} y1={pB.y} x2={padL + 44} y2={pA.y} stroke="var(--ink)" strokeWidth="1.5" />
            <line x1={padL + 40} y1={pB.y} x2={padL + 44} y2={pB.y} stroke="var(--ink)" strokeWidth="1.5" />
            <line x1={padL + 40} y1={pA.y} x2={padL + 44} y2={pA.y} stroke="var(--ink)" strokeWidth="1.5" />
            <text x={padL + 49} y={midY + 3} className="eh-pt">{addRemove} {fmt(dw, 1)} kg</text>
          </g>
        );
      })()}

      {pB && pA && (
        <line x1={pB.x} y1={pB.y} x2={pA.x} y2={pA.y} stroke="var(--ink)" strokeWidth="1.5" strokeDasharray="3 3" markerEnd="url(#arrow2)" />
      )}
      <defs>
      ...
      {pB && (
        <g>
          <circle cx={pB.x} cy={pB.y} r="5" fill={pB.in ? "var(--good)" : "var(--bad)"} stroke="#fff" strokeWidth="1.5" />
          <text x={pB.x + 8} y={pB.y - 6} className="eh-pt">before</text>
        </g>
      )}
      {pA && (
        <g>
          <circle cx={pA.x} cy={pA.y} r="5" fill="var(--amber)" stroke="#fff" strokeWidth="1.5" />
          <text x={pA.x + 8} y={pA.y + 14} className="eh-pt">after</text>
        </g>
      )}
      {!plot && cg0 && w0 && (
        <circle cx={sx(iuFor(w0, cg0))} cy={sy(w0)} r="5"
          fill={(cg0 >= fwdLimit(w0) && cg0 <= ECHO.AFT_LIMIT) ? "var(--good)" : "var(--bad)"}
          stroke="#fff" strokeWidth="1.5" />
      )}

      <text x={padL} y={12} className="eh-axis">Loading chart · weight (kg) vs index units (IU)</text>
    </svg>
  );
}
...
export default function EchoLoadingHelper() {
  ...
  const [graphView, setGraphView] = useState("cg");
  ...
  return (
    <div className="eh-root">
        ...
        <section className="eh-panel">
          ...
          {showGraph && (
            <div className="eh-graphwrap">
              <div className="eh-graphtabs">
                <button className={graphView === "cg" ? "gt on" : "gt"} onClick={() => setGraphView("cg")}>CG vs weight</button>
                <button className={graphView === "iu" ? "gt on" : "gt"} onClick={() => setGraphView("iu")}>Loading chart (IU) · workbook style</button>
              </div>
              {graphView === "cg"
                ? <EnvelopeGraph plot={result.summary.plot} cg0={result.summary.cg0} w0={result.summary.w0} />
                : <IndexGraph plot={result.summary.plot} cg0={result.summary.cg0} w0={result.summary.w0} />}
            </div>
          )}
                  ...
                  {qResult.summary?.plot && (
                    <div className="eh-graphwrap">
                      ...
                      {graphView === "cg"
                        ...
                        : <IndexGraph plot={qResult.summary.plot} cg0={qResult.summary.cg0} w0={qResult.summary.w0} />}
                    </div>
                  )}
                  <div className="eh-steps">
      ...
      <footer className="eh-version">
        {APP_VERSION.tag} · {APP_VERSION.date} · {APP_VERSION.note}
      </footer>
    </div>
  );
}
